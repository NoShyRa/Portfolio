...
(function($) {
	...
	// Main.
		...
		// search
			...
			const searchResults = document.getElementsByClassName("search-tab");

			searchInput.addEventListener("keyup", function() {
			  const searchValue = searchInput.value.toLowerCase();
			  for (let i = 0; i < searchResults.length; i++) {
			    if (searchResults[i].textContent.toLowerCase().includes(searchValue)) {
		...
		//PDF
			...
			pdfs.forEach(pdf => {
			  const loadPdf = document.getElementById(pdf.id);
			  const pdfFrame = document.getElementById(pdf.frameId);

			  loadPdf.addEventListener("click", function() {
			    pdfFrame.src = pdf.src;
